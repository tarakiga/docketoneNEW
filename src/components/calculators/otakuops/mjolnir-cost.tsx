...
export function MjolnirArmorCostCalculator() {
    ...
    return (
        <div className="grid lg:grid-cols-3 gap-8">
            <Card className="glass-card lg:col-span-1 h-fit min-w-0" style={{ backgroundColor: 'var(--dk-surface)', borderColor: 'var(--dk-line)' }}>
                <CardHeader>
                    ...
                    <CardTitle className="flex items-center gap-2" style={{ color: 'var(--dk-ink)' }}>
                        <Shield className="w-5 h-5" style={{ color: 'var(--dk-yel-ink)' }} />
                        Requisition Form
                    </CardTitle>
                </CardHeader>
                <CardContent className="space-y-6">
                    <div className="space-y-2">
                        <Label className="text-xs font-bold uppercase tracking-wider" style={{ color: 'var(--dk-ink-soft)' }}>Armor Generation</Label>
                        <Tabs defaultValue="mk5" onValueChange={(v) => setGeneration(v as "mk4" | "mk5" | "mk6")} className="w-full">
                            <TabsList className="grid w-full grid-cols-3" style={{ backgroundColor: 'var(--dk-sunk)' }}>
                                <TabsTrigger value="mk4" className="!text-[var(--dk-ink-soft)] !bg-[var(--dk-raised)] border border-[var(--dk-line)] data-[state=active]:!bg-[var(--dk-yel)] data-[state=active]:!text-[var(--dk-on-fill)]">Mk IV</TabsTrigger>
                                <TabsTrigger value="mk5" className="!text-[var(--dk-ink-soft)] !bg-[var(--dk-raised)] border border-[var(--dk-line)] data-[state=active]:!bg-[var(--dk-yel)] data-[state=active]:!text-[var(--dk-on-fill)]">Mk V</TabsTrigger>
                                <TabsTrigger value="mk6" className="!text-[var(--dk-ink-soft)] !bg-[var(--dk-raised)] border border-[var(--dk-line)] data-[state=active]:!bg-[var(--dk-yel)] data-[state=active]:!text-[var(--dk-on-fill)]">Mk VI</TabsTrigger>
                            </TabsList>
                        </Tabs>
                    </div>

                     <div className="space-y-2">
                        <Label className="text-xs font-bold uppercase tracking-wider" style={{ color: 'var(--dk-ink-soft)' }}>Your Net Worth (cR)</Label>
                        <Input type="number" min={0} value={userNetWorth} onChange={(e) => { const v = Number(e.target.value); setUserNetWorth(Number.isFinite(v) ? Math.max(0, v) : 0) }} style={{ backgroundColor: 'var(--dk-sunk)', borderColor: 'var(--dk-line)', color: 'var(--dk-ink)' }} />
                    </div>
                </CardContent>
            </Card>
            ...
            <div className="lg:col-span-2 space-y-6 min-w-0">
                <Card className="glass-card border-none p-1" style={{ backgroundColor: 'var(--dk-surface)' }}>
                     <CardContent className="pt-12 pb-16 text-center">
                        <div className="text-[10px] uppercase tracking-widest mb-2" style={{ color: 'var(--dk-ink-soft)' }}>Estimated Cost</div>
                        <div className="text-2xl sm:text-4xl md:text-6xl font-black mb-2 break-words leading-tight px-2" style={{ fontFamily: 'var(--font-fredoka), cursive', color: 'var(--dk-yel-ink)' }}>{formattedCost}</div>
                        <div className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider mb-8" style={{ backgroundColor: 'var(--dk-raised)', borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--dk-line)', color: 'var(--dk-yel-ink)' }}>
                            From the HALO Lore
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-md mx-auto">
                            <div className="min-w-0 p-4 rounded-2xl" style={{ backgroundColor: 'var(--dk-sunk)', borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--dk-line)' }}>
                             ...
                             <div className="min-w-0 p-4 rounded-2xl" style={{ backgroundColor: 'var(--dk-sunk)', borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--dk-line)' }}>
                                ...
                                <div className="text-lg md:text-xl font-bold" style={{ color: 'var(--dk-yel-ink)' }}>{carrierGroups.toFixed(1)}</div>
                            </div>
                        </div>
