...
const K = "text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--dk-ink-soft)] font-mono mb-2"
const TILE = "bg-[var(--dk-surface)] border border-[var(--dk-line)] rounded-2xl p-5 flex flex-col justify-between"
...
export function CaffeineStatusCard({
    ...
    const loadColor = status === "active" ? "text-[var(--dk-neg-ink)]" : status === "moderate" ? "text-[var(--dk-warn-ink)]" : "text-[var(--dk-pos-ink)]"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <motion.div className={TILE} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                ...
                <div className={`text-5xl font-black tracking-tighter ${loadColor} flex items-baseline gap-1`}>
                    {currentLevel.toFixed(0)}<span className="text-lg text-[var(--dk-ink-soft)] font-medium">mg</span>
                </div>
                <div className="flex items-center gap-2 mt-3 text-sm font-semibold text-[var(--dk-ink)]">
            ...
            <motion.div className={TILE} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.08 }}>
                ...
                <div className={`text-5xl font-black tracking-tighter flex items-baseline gap-1 ${isSleepImpacted ? "text-amber-400" : "text-[var(--dk-pos-ink)]"}`}>
                    {bedtimeLevel.toFixed(0)}<span className="text-lg text-[var(--dk-ink-soft)] font-medium">mg</span>
                </div>
                <div className="text-xs text-[var(--dk-ink-soft)] mt-3">
            ...
            <motion.div className={TILE} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.16 }}>
                ...
                <div className="text-4xl font-bold font-mono tracking-tight text-[var(--dk-tea-ink)]">{safeLabel}</div>
                <div className="text-xs text-[var(--dk-ink-soft)] mt-3">when you drop under {threshold} mg</div>
            </motion.div>
        </div>
    )
}
