...
export function NewsletterForm({ className, dark }: NewsletterFormProps) {
  ...
  return (
    <div className={cn(dark ? "home-news" : "almanac-callout", className)}>
      <h2 style={{ fontFamily: dark ? "var(--font-fredoka), cursive" : "var(--font-fraunces), serif", fontWeight: dark ? 600 : 600, fontSize: dark ? 30 : 24, color: dark ? "var(--dk-yel)" : "var(--ink)", marginBottom: 6 }}>
        {dark ? "Join the calculation nation" : "Stay calculated"}
      </h2>
      <p style={{ color: dark ? "#FFFFFF" : "var(--ink-soft)", fontSize: 15, marginBottom: 16 }}>
      ...
      >
        <input
          ...
          style={{ flex: "1 1 220px", ...(dark ? { background: "var(--dk-surface)", borderColor: "var(--dk-line)", color: "var(--dk-ink)" } : {}) }}
          value={email}
