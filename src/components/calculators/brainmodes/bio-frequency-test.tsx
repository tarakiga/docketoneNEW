...
export function BioFrequencyTest() {
    ...
    const MONO_K = "font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--dk-ink-soft)]"
    ...
    return (
        ...
        >
            <div className="relative z-10">
                ...
                {/* Volume note */}
                <div className="rounded-xl px-4 py-3 text-[12.5px] text-[var(--dk-ink-soft)] max-w-2xl mx-auto flex gap-2.5 mb-6" style={{ background: "var(--dk-sunk)", border: "1px solid var(--dk-line)" }}>
                    <span>🔉</span>
                    <span><b className="text-[var(--dk-ink)]">Lower your volume to ~20%</b> before testing. One tone at a time - hold the pad, listen carefully, then answer honestly.</span>
                </div>

                {phase === "gate" && (
                    <div className="max-w-md mx-auto text-center rounded-2xl p-8 my-4" style={{ background: "var(--dk-sunk)", border: "1px solid var(--dk-line)" }}>
                    ...
                    <>
                        ...
                        <div className="text-center max-w-xl mx-auto">
                            ...
                            <div className="relative w-[190px] h-[190px] mx-auto my-8 grid place-items-center select-none">
                                <div className="absolute inset-[-38px] rounded-full border pointer-events-none" style={{ borderColor: "var(--dk-line)" }} />
                                <div className="absolute inset-[-18px] rounded-full border pointer-events-none" style={{ borderColor: "var(--dk-line)" }} />
                                <div className="absolute inset-0 rounded-full border pointer-events-none" style={{ borderColor: "var(--dk-line)" }} />
                                <button
                                    type="button"
                    ...
                    <div className="text-center max-w-2xl mx-auto">
                        <div className="rounded-2xl p-8 mb-4" style={{ background: "var(--dk-sunk)", border: "1px solid var(--dk-line)" }}>
                            ...
                            <div className="font-mono text-[12px] text-[var(--dk-ink-soft)]">
                                {maxHeard > 0
                                    ? <>Top confirmed frequency: <b className="text-[var(--dk-ink)]">{maxHeard.toLocaleString()} Hz</b></>
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-4">
                            <div className="rounded-2xl p-4 text-left" style={{ background: "var(--dk-sunk)", border: "1px solid var(--dk-line)" }}><div className={MONO_K}>👂 Sensitivity</div><div className="font-extrabold text-[var(--dk-ink)] text-xl mt-1">{sensitivity}</div></div>
                            <div className="rounded-2xl p-4 text-left" style={{ background: "var(--dk-sunk)", border: "1px solid var(--dk-line)" }}><div className={MONO_K}>🎯 Tones confirmed</div><div className="font-extrabold text-[var(--dk-ink)] text-xl mt-1">{heardCount} of {FREQUENCIES.length}</div></div>
                        </div>

                        <div className="flex gap-3 justify-center flex-wrap">
                            <button onClick={retake} className="font-mono font-bold text-[13px] uppercase tracking-wide text-[var(--dk-ink)] rounded-xl px-6 py-3 transition-colors" style={{ background: "var(--dk-raised)", border: "1px solid var(--dk-line)" }}>↺ Test again</button>
                            <ShareResult
                                title="Ear Age Test"
                                text={maxHeard > 0
                ...
                <p className="text-[11px] text-[var(--dk-ink-soft)] leading-relaxed text-center max-w-2xl mx-auto mt-6">
